...
void sortedInsert_artist(struct artist_list** head_ref, struct artist_list* new_artist){
	...
	if(*head_ref == NULL || (*head_ref)->tid > new_artist->tid){
		new_artist->next_artist = *head_ref;
		*head_ref = new_artist;
	}else{
		current = *head_ref;
		while(current->next_artist != NULL && 
				current->next_artist->tid < new_artist->tid){
...
void delete_artist(struct artist_list** head_ref, pthread_t tidX){
	...
	if(temp != NULL && temp->tid == tidX){
		*head_ref = temp->next_artist;
		free(temp->status);
		free(temp);
		return;
	}
	while(temp != NULL && temp->tid != tidX){
		prev = temp;
		temp = temp->next_artist;
	}
